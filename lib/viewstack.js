	...
	var ViewStack = Backbone.View.extend({
		...
		pushView: function pushView( newView, options ){
			...
			if ( options.animatePreviousView === true ) {
				...
				if (oldActiveView && oldActiveView.onDeactivate)
		...
		popViewToIndex: function popViewToIndex(index, options) {
			if (!_.isNumber(index)) return this;
			options = _.defaults(options || {}, {
				animateLastView: false
			});
			var initialSize = this.size();
			var finalSize   = index + 1;
			var poppedViews = 0;
			while (this.size() > finalSize && poppedViews < (initialSize-finalSize)) {
				var element = this._stack[finalSize];
				element.options.animatePreviousView = false;
				if ((this.size() == finalSize+1) && options.animateLastView) {
					var self = this;
					window.requestAnimationFrame(function() {
						self.popView(element.view, { animated: true });
					});
				} else {
					this.popView(element.view, { animated: false });
				}
				poppedViews++;
			}
		...
		_popView: function _popView(poppedView, options) {
			...
			if ( _.isObject(el) && el.view && this._current != el.view){
		...
		_hookActivate: function _hookActivate(view, animate, firstTime) {
			...
			if ( view.onBeforeActivate )
				view.onBeforeActivate(firstTime, animate);
