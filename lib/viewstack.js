...
var ViewStack = module.exports = Backbone.View.extend({
	...
	pushView: function pushView( newView, options ){
		...
		//
		// CurrentView
		// onBeforeDeactivate(modal)
		//      animate
		// onDeactivate(modal) ... In this hook deactivate all events, scroll, ecc
		//
	...
	popView: function popView(poppedView, options){
		...
		if ( !options.animated ){
			poppedView = self._popView(poppedView);
			runPopHook();
			return poppedView;
		}
	...
	_popView: function _popView(poppedView) {
